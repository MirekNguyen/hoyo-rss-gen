...
for route_name, url in config.data.get("routes").items():
    ...
    controller_name = ''.join(word.capitalize() for word in base_name.split('_')) + 'Controller'
    controller = globals().get(controller_name)
    if controller and callable(controller):  # Check if the function exists and is callable
        webscrape = controller(url)
    else:
        raise ValueError(f"No controller found for route '{route_name}'")
if webscrape is None:
    raise ValueError(f"No valid controller found for the provided route: '{config.args.route}'")
